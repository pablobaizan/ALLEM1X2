...
public class Calc extends Thread {
	...
	private ConcurrentHashMap<Integer, float[]> mem;
	private Pool pool;
	...
	public Calc(Config config) {
		...
		this.mem = new ConcurrentHashMap<Integer, float[]>();
		...
		try {
			if(this.config.getFileSaved() != null) {
				...
				while(br.ready()) {
					...
					c.setP14(Float.parseFloat(pieces[1]));
					c.setMad(Float.parseFloat(pieces[2]));
					c.setKelly(Float.parseFloat(pieces[3]));
					c.setTvm(Float.parseFloat(pieces[4]));
					c.setEms(new float[] {
							Float.parseFloat(pieces[5]), 
							Float.parseFloat(pieces[6]), 
							Float.parseFloat(pieces[7]), 
							Float.parseFloat(pieces[8]), 
							Float.parseFloat(pieces[9])
							});
	...
	public void consolePrint(boolean aceptada, Combinacion c) {
		String str = "";
		float emar = c.getEms()[0] + c.getEms()[1] + c.getEms()[2] + c.getEms()[3];
		if(aceptada) {
			str += "Aceptada ";
		}
		else {
	...
	public boolean checkEM(float[] ems) {
		float emt = 0.0f;
		for(int i = 0; i < 5; i++) {
	...
	public void print() {
		try {
			...
			if(this.config.getFileCSV() != null) {
				...
				bwCSV.write("Columna" + "\t" + "Prob14" + "\t" + "Mad" + "\t" + "Kelly" + "\t" + "TVM" + "\t" + "EM10" + "\t" + "EM11" + "\t" + "EM12" + "\t" + "EM13" + "\t" + "EM14");
				bwCSV.newLine();
				for(Combinacion c : listaOrdenada) {
					bwCSV.write(c.toString());
					bwCSV.write("\t" + c.probabilidad14());
					bwCSV.write("\t" + c.getMad());
					bwCSV.write("\t" + c.getKelly());
					bwCSV.write("\t" + c.getTvm());
	...
	public ConcurrentHashMap<Integer, float[]> getMem() {
		return this.mem;
	}
